...
void *pila_desapilar(pila_t *pila) {
    ...
    size_t nueva_dimension = pila->capacidad / FACTOR_REDIMENSION;
    if(
        (pila->cantidad * DIFERENCIA_CAPACIDAD <= pila->capacidad) && 
        (nueva_dimension >= CAPACIDAD_INICIAL)
    ) { 
        pila_redimensionar(pila, nueva_dimension);
    };
